...
def extract_entity(parse_tree, semantic_role='', verbose=False):
    ...
    if semantic_role == 'Theme':
        ...
        #count number of NNs (e.g. bomb, hostage, etc.) in tree
        for position in parse_tree.treepositions():
            if not isinstance(parse_tree[position], Tree):
                continue
            print position
            if parse_tree[position].node == "NN":
                entities.append(ObjectEntity()) 
    else:
    """
...
def create_semantic_structures(frame_semantic_list):
    ...
    for frame in frame_semantic_list:
        ...
        if not isinstance(frame, FrameMatch):
            if frame in ["and", "if"]:
                continue
            else: raise Exception("Unrecognized format for frame {}. Frame must be an instance of FrameMatch".format(frame))
        ...
        # It's a regular command
            ...
            if frame.condition:
                ...
                current_command.condition = _make_command(frameaction, frame.condition, condition = True)
...
def _make_command(action, frame, condition = False):
    ...
    try:
        if isinstance(frame.args['Theme'], Tree):
            theme_tree = frame.args['Theme']
            themes = []
            for position in theme_tree.treepositions():
                if not isinstance(theme_tree[position], Tree):
                    continue
                if theme_tree[position].node == "NN":
                    themes.append(theme_tree[position[:-1]])
                elif theme_tree[position].node == "PRP":
                    themes.append(frame.condition.args['Theme'])
            ...
            for t in themes:
                frame.args['Theme'] = t
                item_to_entity.append(_framearg_entities(frame.args))

    except KeyError:
        #This is used for sentences such 
        #as 'If you see a hostage, interact with him'
        #in this case the syntactic parser returns a Frame 
        #which doesn't have any 'Theme' inside
        #This snippet overrides the problem but it only works
        #with verb 'interact'
        #TODO: make this more robust and extend to other verbs
        if "interact" in frame.args['VERB'].leaves():
            themes = []
            themes.append(frame.condition.args['Theme'])
    ...
    #ADDED
    theme = []
    for item in item_to_entity:
        theme.append(item.get('Theme', None))
    item_to_entity = item_to_entity[0]

    #DEBUG
    #print "ITEM TO ENTITY: "
    #print item_to_entity
    #print

    #theme = item_to_entity.get('Theme', None) #ORIGINAL
    agent = item_to_entity.get('Agent', None)
    #ADDED
    #sometimes instead of 'agent' 
    #the word 'actor' is returned
    #by the syntactic parser
    if agent is None:
        agent = item_to_entity.get('Actor', None)
    ...
    for vp, parent in vps:
        ...
        if support_verb:
            ...
            for i, child in enumerate(vp):
                if child.node.startswith(VERB_TAG) or child.node.startswith(VERB_TAG2): #CHANGED
...
def split_conjunctions_sparse(parse_tree):
        ...
        for tree_pos in parse_tree.treepositions():
            ...
            if "SBAR-ADV" in parse_tree[tree_pos].node:
                ...
                if num_ands > 0:
                    ...
                    trees = []
                    for temp_tree in splitter.split_on_multiple_ccs(parse_tree, "or"):
                        for tree in splitter.split_on_multiple_ccs(temp_tree, "and"):
                            trees.append(tree)
                else:
    ...
    for i, item in enumerate(trees): 
        ...
        g2.render("/home/gian/hri_project/figures/parse_trees/splitted_command{}.gv".format(str(i)))
