...
from ltlbroom.ltl import (
    env, and_, or_, sys_, not_, iff, next_, parens, always, always_eventually, implies,
...
class SpecGenerator(object):
    ...
    def generate(self, text, sensors, regions, props, tag_dict, realizable_reactions=True):
        ...
        if realizable_reactions and self.react_props:
            ...
            #CHANGED
            reaction_or_frag = and_([sys_(prop) for prop in self.react_props])

            #DEBUG
            #print "REACTION OR FRAG: ", reaction_or_frag
    ...
    def _apply_metapar(self, command):
        ...
        if (command.action not in self.GOALS and
            ...
            if command.negation:
                command.theme =[ObjectEntity("not_" + command.action)]
                command.negation = False
            else:
        ...
        if command.condition:
            #CHANGED
            return self._gen_multi_conditional(command)
            #return self._gen_conditional(command) #ORIGINAL
        else:
    ...
    def _gen_multi_conditional(self, command, assume_eventual_relief=False):
        """Generate an action which depends on multiple conditions"""
        ...
        if command_condition.action == SEE_ACTION:
            ...
            #DEBUG
            """
            print "COMMAND CONDITION: "
            print command_condition
            print type(command_condition)
            """

            #ADDED
            conditions = []
            for t in command_condition.theme:
                condition = t.name
                if condition not in self.sensors:
                    raise BadConditionalError(condition)
                conditions.append(condition)

            #CHANGED
            condition_frag = and_([env(c) for c in conditions]).strip("(").strip(")")
            condition_label = and_(conditions).strip("(").strip(")")
            explanation = "To react to {!r},".format(condition_label)
            if assume_eventual_relief:
                relief_explanation = "Assume {!r} eventually goes away.".format(condition_label)
        ...
        if action in self.props:
            ...
            reaction_props = []
            for c in conditions:
                reaction_props.append(sys_(REACT + "_" + c))
                reaction_prop_names.append(REACT + "_" + c)
            reaction_prop = and_(reaction_props).strip("(").strip(")")
            reaction_prop_name = and_(reaction_prop_names).strip("(").strip(")")

            #DEBUG
            """
            print "REACTION PROP NAME: ", reaction_prop_name
            print "REACTION PROP: ", reaction_prop
            """

            #CHANGED
            #new_props.append(reaction_prop_name) #ORIGINAL
            for p in reaction_prop_name.split(" & "):
                new_props.append(p)
                self.react_props.add(p)
        ...
        if action in (GO_ACTION, AVOID_ACTION):
            ...
            #CHANGED
            if command.theme[0].name not in self.props:
                raise BadArgumentError("Unknown actuator {!r}".format(command.theme[0].name))
            # Otherwise we are always creating reaction safety
            sys_statements.append(always(iff(next_(condition_frag), next_(reaction_prop))))
            template = " {} {!r}." if not command.negation else " do not {} {!r}."
            explanation += template.format(action, command.theme[0].name)
            ...
            stay_there = always(implies(or_([parens(reaction_prop), next_(reaction_prop)]),
                                        self._frag_stay()))
    ...
    def _gen_conditional(self, command, assume_eventual_relief=False):
        ...
        if command_condition.action == SEE_ACTION:
            ...
            if not command.condition.theme:
                raise BadArgumentError("No condition theme, cannot understand condition:\n{!r}"
                                       .format(command.condition))
            condition = command.condition.theme[0].name #CHANGED
            if condition not in self.sensors:
                raise BadConditionalError(command.condition.theme[0].name) #CHANGED
        ...
        if action in (GO_ACTION, AVOID_ACTION):
            ...
            if command.theme[0].name not in self.props: #CHANGED
                raise BadArgumentError("Unknown actuator {!r}".format(command.theme.name))
            # Otherwise we are always creating reaction safety
            sys_statements.append(always(iff(next_(condition_frag), next_(reaction_prop))))
            template = " {} {!r}." if not command.negation else " do not {} {!r}."
            explanation += template.format(action, command.theme[0].name) #CHANGED
...
def _frag_react_activate(command):
    ...
    #CHANGED
    if type(command.theme) is list:
        return next_(sys_(command.theme[0].name))
    else:
...
def _frag_react_deactivate(command):
    ...
    #CHANGED
    if type(command.theme) is list:
        return not_(next_(sys_(command.theme[0].name)))
...
def _format_command(command):
    ...
    #CHANGED
    if command.theme != [None]:
        fields.append("Argument: {!r}".format(command.theme))
